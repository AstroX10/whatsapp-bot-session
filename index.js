...
import { zipAndUpload } from './upload.js';
...
// Pairing route (initial pairing code)
...
app.get('/access-key', (req, res) => {
 if (global.accessKey) {
  res.json({ accessKey: global.accessKey });
 } else {
  res.status(404).json({ error: 'Access key not available yet' });
 }
});
